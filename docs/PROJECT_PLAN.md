...
项目使用 Amazon Reviews 2023 的 `Musical_Instruments` 单品类数据。系统结合用户历史行为、当前自然语言需求、结构化约束和真实评论证据，返回带引用的 Top K 商品推荐。当前一期 MVP 已完成推荐与 Evidence 服务层重构、FastAPI 接入、Streamlit 单轮演示页面、阶段级延迟诊断和第一轮低风险延迟优化。
...
结构重构、API 与 UI 接入已经完成，当前一期收尾状态如下：

1. FastAPI：已实现 `/health/live`、`/health/ready` 和 `POST /api/v1/recommend`，通过 Application Service 调用正式推荐链路。
2. Streamlit：已实现只通过 HTTP 调用 FastAPI 的单轮演示页面，展示 Top K、证据和解释。
3. 延迟诊断：已加入阶段级 `cartwise_timing` 日志，并完成第一轮小范围优化。
4. 最终收尾：README 和阶段文档记录复现流程、已知限制、query-level 评估集缺口和后续改进项。
...
- 数据集没有真实导购 query、搜索日志或 query-product 人工相关性标注；自然语言 query-level benchmark 需要后续单独建设。
...
| Popularity | 最低成本基线 |
| LightGCN | 验证个性化图推荐增益 |
| LightGCN + Dense/BM25 + 硬过滤 | 最终一期链路的系统验收与人工检查 |
...
其中 `Recall@10`、`NDCG@10` 和 `HitRate@10` 基于历史交互划分报告。自然语言 query 到
最终推荐结果的严格离线指标暂不纳入一期，因为当前数据没有 query-level 标注集；该任务
作为后续 query-level 评估集建设推进。
